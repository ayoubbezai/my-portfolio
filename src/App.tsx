...
const App = () => {
  ...
  const fallback = (label: string) => (
    <div className="w-full flex justify-center py-20 text-xl text-indigo-500">Loading {label}...</div>
  );
  ...
  return (
    <div className=" relative  overflow-x-hidden  h-full w-full bg-white dark:bg-slate-950">
      ...
      <section id="alert">
        ...
        <Suspense fallback={fallback("Hero")}> <Hero /> </Suspense>
        <Suspense fallback={fallback("About")}> <About /> </Suspense>
        <Suspense fallback={fallback("Technologies")}> <Technologies /> </Suspense>
        <Suspense fallback={fallback("Stats")}> <Stats /> </Suspense>
        {/* <Certifications /> */}
        <Suspense fallback={fallback("Experience")}> <Experience /> </Suspense>
        <Suspense fallback={fallback("Achivements")}> <Achivements isDarkMode={isDarkMode} /> </Suspense>
        <Suspense fallback={fallback("Projects")}> <Projects name="Programming Projects" info={PROJECTS} /> </Suspense>
        {/* <Projects name="Design Projects" info={DESIGN} /> */}
        <Suspense fallback={fallback("Contact")}> <Contact /> </Suspense>
      </section>
    </div>
  );
};
